...
- (void)setModel:(SettingRowModel *)model {
    ...
    switch (model.rowType) {
        case SettingRowTypeCustome: {
            [self.titleLabel removeFromSuperview];
        }
            break;
        case SettingRowTypeArrow: {
            if (self.desLabel) {
            ...
            if (model.imageName && model.imageName.length > 0) {
                ...
                [self.titleView mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.left.mas_equalTo(self.mas_left).offset(YYSIZE_13);
                    make.centerY.mas_equalTo(self.mas_centerY);
                    make.size.mas_offset(CGSizeMake(YYSIZE_20, YYSIZE_20));
                }];
                
                [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.left.mas_equalTo(self.titleView.mas_right).offset(YYSIZE_18);
                    make.centerY.mas_equalTo(self.mas_centerY);
                }];
            } else {
                [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        case SettingRowTypeDescArrow: {
            ...
            if (!self.desLabel) {
                self.desLabel = ({
                    ...
                    v.textColor = COLOR_5d4fe0;
                    v.font = FONT_DESIGN_28;
                    v;
                });
                [self addSubview:self.desLabel];
                [self.desLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.right.mas_equalTo(self.arrowView.mas_left).offset(-YYSIZE_05);
                    make.centerY.mas_equalTo(self.mas_centerY);
                }];
            }
        ...
        case SettingRowTypeCheckbox: {
            ...
            self.checkBtn.selected = _model.selected;
             _titleLabel.textColor = _model.selected ? COLOR_30D1A1 : COLOR_1a1a1a;
            break;
        }
        case SettingRowTypeNone:
            break;
    }
}
