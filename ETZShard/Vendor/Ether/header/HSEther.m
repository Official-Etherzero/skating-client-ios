...
//    __block EtherscanProvider *e = [[EtherscanProvider alloc]initWithChainId:ChainIdHomestead apiKey:@"RCWEX6WYBXMJZHD5FD617NZ99TZADKBEDJ"];
    ...
    __block JsonRpcProvider *e = [[JsonRpcProvider alloc] initWithChainId:ChainIdEasyetz url:[NSURL URLWithString:rpcIP]];
    ...
    [[e estimateGas:transaction] onCompletion:^(BigNumberPromise *proGasPrice) {
        if (proGasPrice.error) {
            NSLog(@"gaslimit 校验失败");
        } else {
            NSLog(@"打印一下 gasprice 的 value 值吧 %@",proGasPrice.value);
        }
    }];
//
    //1 account自己解密
        ...
        if (NSError == nil){
            ...
            [[e getTransactionCount:transaction.fromAddress] onCompletion:^(IntegerPromise *pro) {
                if (pro.error != nil) {
                    ...
                    NSLog(@"3 获取nonce成功 值为%ld",(long)pro.value);
...
//                            transaction.value = b;
                            ...
                            if (contract == nil) {//默认eth
                                ...
                                if (dataStr && dataStr.length > 0) {
                                    ...
                                    [data appendData:[SecureData hexStringToData:dataStr]];
                                ...
                                transaction.data = data.data;
                                transaction.toAddress = [Address addressWithString:contract];//合约地址（代币交易 转入地址为合约地址）
                            }
                            //签名
                            [a sign:transaction];
                            //发送
...
+(void)hs_changePasswordForPrivateKey:(NSString *)privateKey
    ...
    //2 生成keystore
    [account encryptSecretStorageJSON:newPwd callback:^(NSString *json) {
        NSData *jsonData = [json dataUsingEncoding:NSUTF8StringEncoding];
        NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:jsonData
                                                            options:NSJSONReadingMutableContainers
                                                              error:nil];
    ...
    [Account decryptSecretStorageJSON:keyStore password:pwd callback:^(Account *account, NSError *NSError) {
        [[e getTransactionCount:transaction.fromAddress] onCompletion:^(IntegerPromise *pro) {
            if (pro.error != nil) {
                ...
                NSLog(@"获取nonce成功 值为%ld",(long)pro.value);
                transaction.nonce = pro.value;
                transaction.chainId = e.chainId;
                transaction.toAddress = [Address addressWithString:toAddress];
                transaction.value = [[Payment parseEther:money] div:[BigNumber bigNumberWithInteger:pow(10.0, 0)]];
                SecureData *data = [SecureData secureDataWithCapacity:68];
                if (dataString && dataString.length >= 42) {
                    // 有参数
                    [data appendData:[SecureData hexStringToData:dataString]];
                    transaction.data = data.data;
                } else {
                    // 没有参数
                    if (dataString) {
                    ...
                    for (int i=0; i < 42 - dataString.length; i++) {
                        [data appendByte:'\0'];
                    }
                    transaction.data = data.data;
                }
                //签名
                [a sign:transaction];
                [[e estimateGas:transaction] onCompletion:^(BigNumberPromise *proGaslimit) {
                    if (proGaslimit.error) {
                        NSLog(@"gaslimit 校验失败");
                    } else {
                        NSLog(@"打印一下 gaslimit 的 value 值吧 %@",proGaslimit.value);
                    }
                    if (completionCallback) {
                        completionCallback(proGaslimit);
                    }
                }];
            }
        }];
...
+ (void)yy_getGasPriceWithAddress:(NSString *)address
                            rpcIP:(NSString *)rpcIP
               completionCallback:(void (^)(BigNumberPromise *))completionCallback {
    __block JsonRpcProvider *e = [[JsonRpcProvider alloc] initWithChainId:ChainIdEasyetz url:[NSURL URLWithString:rpcIP]];
    
    [[e getGasPrice] onCompletion:^(BigNumberPromise *proGasPrice) {
        if (proGasPrice.error == nil) {
            NSLog(@"获取gasPrice成功 值为%@",proGasPrice.value.decimalString);
            if (completionCallback) {
                completionCallback(proGasPrice);
            }
        }
    }];
}
...
//            [a sign:transaction];
            [[e estimateGas:transaction] onCompletion:^(BigNumberPromise *proGaslimit) {
                if (proGaslimit.error) {
                    if (failure) {
                        failure(proGaslimit.error);
                    }
                } else {
                    if (success) {
                        success(proGasPrice,proGaslimit);
                    }
                }
            }];
        }
...
+ (void)yy_getNodeBlockNumberWithRpcIP:(NSString *)rpcIP {
    ...
    [[e getBlockNumber] onCompletion:^(IntegerPromise *promise) {
        // 获取当前的区块高度
        NSLog(@"rpcIp:%@，区块高度:%@", rpcIP,promise);
    }];
}
